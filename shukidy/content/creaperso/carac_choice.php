<div class="ventreBox">
	...
	<table>
		...
		$caracCount = 0;
		foreach ($caracOfUniv as $key => $carac) { $key++;
		if ($carac['name'] !== ''){
			$caracCount++;
			?>
			...
			<tr class="caracBox">
				...
				<td>
					<div class="displayCarac-container">
						<div class="displayCarac-icon" style="background-image: url('img/gameicons/<?=$carac['icon']?>');background-color: <?=$carac['color']?>" ></div>
						<div class="displayCarac-value" carac="<?=$key?>">1</div>
					</div>
				</td>
				<td>
					<input class="slider" type="range" min="1" max="10" value="1" name="c<?=$key?>_value" carac="<?=$key?>">
				</td>
			</tr>
